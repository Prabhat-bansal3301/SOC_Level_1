...
## Wireshark — Decrypting HTTPS/TLS Traffic

### Why HTTPS Decryption Matters
- Attackers and malware also use HTTPS — encrypted ≠ safe
- Without decryption: only see IP/port info, not URLs or content
- With key log file: full visibility into requests, responses, transferred data

---

### HTTPS/TLS Wireshark Filters
...
ssdp                            # SSDP discovery protocol (filter out as noise)
```

**TLS Handshake Investigation:**
```wireshark
# Client Hello (who is initiating TLS connections)
(http.request or tls.handshake.type == 1) and !(ssdp)

# Server Hello (which servers are responding)
(http.request or tls.handshake.type == 2) and !(ssdp)
```

<img width="1668" height="1069" alt="ssdp1" src="https://github.com/user-attachments/assets/a3a4c324-7e87-4e37-8dc2-c391e1b1d2ee" />

<img width="1668" height="1068" alt="ssdp2" src="https://github.com/user-attachments/assets/55f9a940-95c6-43e2-a807-9acc91c4c4f9" />

---

### TLS Handshake vs TCP 3-Way Handshake

```
TCP:  SYN → SYN,ACK → ACK          (connection established)
TLS:  Client Hello → Server Hello → [Certificate exchange → Key exchange]
```

Client Hello + Server Hello = identify which IPs are in TLS communication.

---

### SSL Key Log File

**What it is:** Text file containing unique key pairs per TLS session.
**Generated by:** Browser (Chrome/Firefox) when configured.
**Required for:** Decrypting captured TLS/HTTPS traffic.

**Critical requirement:** Keys must be captured **during** the traffic capture.
Keys are generated per session — you cannot decrypt traffic retroactively without the keys from that exact session.

---

### Setting Up SSL Key Logging

```bash
# Linux/Mac — set environment variable before launching browser
...
### Loading Key Log File in Wireshark

**Method 1 — Right-click on TLS packet:**

<img width="1656" height="1033" alt="Adding key log files with the (right-click) menu" src="https://github.com/user-attachments/assets/2e348791-1265-4f85-b002-25f757ebba88" />

```
Right-click TLS packet → Protocol Preferences → TLS → Pre-Master Secret log filename
```

**Method 2 — Preferences menu:**
```
Edit → Preferences → Protocols → TLS → Pre-Master Secret log filename → Browse
```

<img width="1656" height="1033" alt="adding key logs" src="https://github.com/user-attachments/assets/196c07fe-885a-4bb2-bdc2-4bde687d7b83" />
...
| | Without Key Log | With Key Log |
|-|-----------------|-------------|
| Protocol shown | TLS (encrypted) | HTTP/2 (decrypted) |
| URLs visible | No | Yes |
| Payload visible | No | Yes |
| Packet color | Different (encrypted) | Normal HTTP colors |

---

### Decrypted Data Formats Available

After loading key log file, packet details pane shows:

| Format | Contains |
|--------|---------|
| **Frame** | Raw frame data |
| **Decrypted TLS** | Decrypted TLS record |
| **Decompressed Header** | HTTP/2 headers decompressed |
| **Reassembled TCP** | Full TCP stream reassembled |
| **Reassembled SSL** | Full SSL data reassembled |
...
```
Capture traffic (with SSLKEYLOGFILE set in browser)
    ↓
Filter: tls.handshake.type == 1  → identify TLS connections
    ↓
Load key log file:
Edit → Preferences → Protocols → TLS → key log file path
    ↓
Filter: http.request → now see decrypted URLs
    ↓
Follow HTTP/2 streams → inspect full request/response content
```
---

## Wireshark — Cleartext Credential Detection

### What It Is
Wireshark can automatically extract cleartext credentials from certain protocols.
Useful for: detecting credential exposure, brute-force hunting, insecure protocol usage.

> Wireshark is NOT an IDS — this is a helper feature, not automatic detection.
...
<img width="853" height="334" alt="Credential" src="https://github.com/user-attachments/assets/4b96f284-2cf1-4e8f-a4ac-1b0c7902e71c" />

**Requires:** Wireshark v3.1 or later.

---

### Supported Protocols (Auto-extraction)
`FTP` `HTTP` `IMAP` `POP` `SMTP`

> Only these dissectors are programmed to extract cleartext passwords.
> Always do manual checks — don't rely solely on this feature.

---

### Credentials Window — What It Shows

| Column | Content |
|--------|---------|
| **Packet number** | Packet containing the password |
| **Protocol** | Which protocol exposed the credential |
| **Username** | Extracted username |
| **Additional info** | Packet number containing the username |

**Interactive:** Click packet number → jumps to password packet. Click username → jumps to username packet.

---

### Manual Cleartext Credential Filters
...
### Brute-Force Detection vs Mistyped Password

Challenge: Multiple login failures look identical whether brute-force or honest mistakes.

**Indicators of brute-force:**
```
→ High volume of failed logins in short timeframe
→ Sequential or dictionary-pattern usernames/passwords
→ All attempts from same source IP
→ Inhuman speed between attempts
→ Hundreds of PASS commands in FTP
```

**Workflow:**
```
Tools → Credentials → view all extracted credentials
    ↓
Count failed vs successful logins
    ↓
If high failure count from single source → brute-force
    ↓
Follow stream → see exact credentials attempted
    ↓
Note source IP + targeted account → escalate
```

---

### Key Reminder
> Cleartext credentials in traffic = serious security issue regardless of attack.
> Protocols like FTP, HTTP, POP, SMTP transmit credentials in plaintext.
> Finding these in a capture → recommend switching to encrypted alternatives: FTP → SFTP/FTPS | HTTP → HTTPS | POP/IMAP → POP3S/IMAPS | SMTP → SMTPS

---

## Wireshark — Generating Firewall Rules

### What It Does
Converts packet details directly into ready-to-implement firewall rules.
Skip manual rule writing — Wireshark generates the syntax for you.

```
Tools → Firewall ACL Rules
```

<img width="879" height="422" alt="Actionable Results!" src="https://github.com/user-attachments/assets/90e756ca-421e-45f8-87fe-c53fe0a3cefb" />


> Rules generated for implementation on **outside firewall interface**.
...
| Platform | Use case |
|----------|---------|
| **Netfilter (iptables)** | Linux firewalls |
| **Cisco IOS Standard** | Cisco routers — basic ACLs |
| **Cisco IOS Extended** | Cisco routers — advanced ACLs (src+dst+port) |
| **IP Filter (ipfilter)** | BSD systems |
| **IPFirewall (ipfw)** | FreeBSD/macOS |
| **Packet Filter (pf)** | OpenBSD/FreeBSD |
| **Windows Firewall (netsh)** | Windows — new and old format |
...
```
Investigate PCAP → identify malicious IP/port/MAC
    ↓
Select suspicious packet
    ↓
Tools → Firewall ACL Rules
    ↓
Choose firewall platform
    ↓
Copy generated rule → implement on firewall
```
...
| Rule Based On | Blocks |
|---------------|--------|
| **IP address** | All traffic from/to malicious IP |
| **Port** | Specific port-based traffic |
| **MAC address** | Link-layer blocking |
| **Combination** | IP + port (most specific) |
...
**iptables (Linux):**
```bash
iptables -A INPUT -s 192.168.1.25 -j DROP
...
```
netsh advfirewall firewall add rule name="Block Attacker" dir=in action=block remoteip=192.168.1.25
```

---

### Investigation → Action Pipeline

```
1. Detect anomaly (ARP poison, port scan, malware C2)
    ↓
2. Identify attacker IP/port/MAC from PCAP
    ↓
3. Document findings (5 Ws)
    ↓
4. Tools → Firewall ACL Rules → generate block rule
    ↓
5. Implement on perimeter firewall
    ↓
6. Monitor for continued activity
    ↓
7. Escalate to IR team if breach confirmed
```

> Wireshark bridges analysis and response.
> From packet-level detection to firewall implementation in minutes.
